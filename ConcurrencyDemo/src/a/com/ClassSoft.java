...
public class ClassSoft {
	...
	public static void main(String args[]) throws InterruptedException {
		...
		Referred strong = new Referred();// new object
		SoftReference<Referred> soft = new SoftReference<Referred>(strong);

		// Attempt to claim a suggested reference.
		ClassSoft.collect();

		System.out.println("Removing reference first without null");
		// The object may but highly likely wont be collected.
		strong = null;
		ClassSoft.collect();
		System.out.println("Removing reference first with null");

		System.out.println("Consuming heap");
		int count = 0;
		try {
			// Create lots of objects on the heap
			List<ClassSoft> heap = new ArrayList<ClassSoft>(100);
			while (true) {
				count++;
